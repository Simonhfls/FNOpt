...
class Rollout():
    ...
    def initializeMesh(self):
        ...
        if self.input_data and self.input_data['motion_code']:
            ...
            if self.input_data['mode'] == 'arbitrary':
                handle_traj = setup_handle_traj(verts, self.input_data['motion_code'], list(reversed(handle_ind_list)),params.inference.rollout.framerate)
    ...
    def step(self):
        ...
        if params.inference.rollout.wind_density > 0.0:
            ...
            cloth_f_area = get_face_areas_batch(vertices=self.original_dataset.x.permute(0, 2, 3, 1).reshape(1, -1, 3),
                                                faces=self.faces) / (self.length_conversion ** 2) # compute face area
            ...
            a_ext = a_ext + wind_force.reshape(1, self.h, self.w, 3).permute(0, 3, 1, 2)
        ...
        if (self.t_iter + 1) % params.inference.iterations_per_timestep == 0:  # visualize only at a new timestep (a timestep can take several iterations to optimize)
            ...
            # self.predicted_x[self.frame_counter] = x
...
def main():
    ...
    for file_name, motion in task_list:
        ...
        epoch = opt.load_index
        res = opt.mesh_resolution
        ...
        if evaluate:
            log_path = Path(f"evaluation/S_MGNRP/eval_log_{params.net.name}{opt.mesh_resolution}.txt")
        ...
        if do_rollout:
            ...
            print(f"Done in {duration: .2f} s, per frame: {per_frame_duration:.2f}s")
        ...
        if params.inference.rollout.save_render_metamizer:
            seq_name = opt.scene_parameters["scene"]
            if motion:
            ...
            result_file = f"{input_data['obj_code']}_{input_data['motion_code']}_e_{epoch}_Y{Y}_S{S}_B{B}_WD{WD}_iter{iter}_res{res}.mp4"
            save_path = os.path.join(save_dir, result_file)

            if os.path.exists(save_path):
                print(f"[Skip] Render file already exists: {os.path.abspath(save_path)}")
            else:
                pos = torch.from_numpy(npy_results['position']).to(device)
                pos = transform_positions(pos.permute(0, 2, 1), opt.R12, opt.T12).cpu().numpy()
                x_list = [frame.T.reshape(3, opt.h, opt.w) for frame in pos]  # list of (3, H, W)
                bc_mask_list = [opt.original_dataset.bc_masks[0, 0].cpu().numpy()] * len(x_list)  # assuming same bc mask for all frames
                print(f"Saving rendering to: {os.path.abspath(save_path)}")

                render_metamizer_video(
                    x_list=x_list,  # list of (3, H, W)
                    bc_mask_list=bc_mask_list,  # list of (H, W)
                    save_path=save_path,
                    dpi=opt.dpi,
                    azim=-60,
                    elev=30,
