...
from sft.render import render_single,render_metamizer_video_with_gt
...
def main():
    ...
    for file_name, motion in task_list:
        ...
        if params.name == 'abl_optimizers':
            ...
            # result_file_bone_name = f"template_mgnrp_quad_{res}_{input_data['motion_code']}_e_{epoch}"
            result_file_bone_name = f"{opt.scene_parameters['mesh_file'].rpartition('_')[0]}_{res}_{input_data['motion_code']}_e_{epoch}"
            if opt.speed_factor is not None:
                result_file_bone_name += f'_speed{opt.speed_factor}'
            if 'bc_desc' in opt.scene_parameters:
                result_file_bone_name = f"template_mgnrp_quad_{res}_bc_{opt.scene_parameters['bc_desc'].replace('-', '_')}_{input_data['motion_code']}_e_{epoch}"
        else:
            result_file_bone_name = f"{input_data['obj_code']}_{input_data['motion_code']}_e_{epoch}_Y{Y}_S{S}_B{B}_WD{WD}_iter{iter}_res{res}"
            if opt.speed_factor is not None:
                result_file_bone_name += f'_speed{opt.speed_factor}'
            if 'bc_desc' in opt.scene_parameters:
        ...
        if evaluate:
            log_path = Path(f"evaluation/S_MGNRP/eval_log_{params.net.name}{opt.mesh_resolution}_{params.inference.postfix}.txt")
        ...
        if opt.evaluate:
            ...
            try:
                ...
                quad_verts = torch.from_numpy(quad_verts).to(device)
                """
                ...
                print(f'Frame {i}, min: {predicted_pos.min().item()}, max: {predicted_pos.max().item()}')
