...
class Optimization():
    ...
    def initializeOptimization(self):
        ...
        self.original_dataset.reset0_sft_env(self.x)        # todo check, no need ?
    ...
    def processImages(self, image):
        ...
        if self.debug and (self.epoch_counter % params.inference.sft.debug_n_save == 0 or self.epoch_counter == params.inference.sft.n_epochs_opt - 1):
            ...
            img = Image.fromarray((img * 255).astype(np.uint8))
            img.save(os.path.join(self.dir_debug, f'02_predicted_image_{str(self.frame_counter).zfill(3)}.png'))
            ...
            with torch.no_grad():
                ...
                if self.evaluate:
                    if self.real_scene:
                        ...
                        if self.epoch_counter == params.inference.sft.n_epochs_opt - 1:
                            torch.save(self.point_clouds["ours"][self.frame_counter].clone(),
                                       self.scene_parameters["result_point_cloud_files"] + str(self.frame_counter).zfill(
                                           3) + ".pt")
            ### DIFFRAST
            if self.renderer == "pytorch3d":
